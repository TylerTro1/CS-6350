...
def cross_validation(cv_folds: list, depth_limit_values: list, ig_criterion: str = 'entropy') -> Tuple[dict, float]:
    ...
    for depth_limit in depth_limit_values:
        ...
        for i in range(len(cv_folds)):
            fold = cv_folds[i]
            train_folds = pd.concat([cv_folds[j] for j in range(len(cv_folds)) if j != i], axis = 0, sort = False, ignore_index = True) #axis = 0 or axis = 1?

            train_x, train_y = train_folds.drop(columns=['label']), train_folds['label'].tolist()
            validation_x, validation_y = fold.drop(columns=['label']), fold['label'].tolist()

            print(f'Training Folds = {train_folds}')

            tree = DecisionTree(depth_limit, ig_criterion)
            train(tree, train_x, train_y)

            accuracy = evaluate(tree, validation_x, validation_y)
