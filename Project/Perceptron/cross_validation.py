...
def cross_validation(
    ...
    #             x_val = val_fold.drop(columns='label').to_numpy()
    #             y_val = val_fold['label'].to_numpy()

    #             model = init_perceptron(perceptron_variant, num_features=x_train.shape[1], lr=lr, mu=mu)
    ...
    hyperparam_combinations = itertools.product(lr_values, mu_values)
    
    for lr, mu in hyperparam_combinations:
        fold_accuracies = []
        
        for i in range(len(cv_folds)):
            validation_fold = cv_folds[i]
            training_folds = pd.concat([cv_folds[j] for j in range(len(cv_folds)) if j != i], axis = 0, sort = False, ignore_index = True)
            
            y_train = training_folds['label'].to_numpy()
            x_train = training_folds.drop(columns='label').to_numpy()
            
            y_val = validation_fold['label'].to_numpy()
            x_val = validation_fold.drop(columns = 'label').to_numpy()
