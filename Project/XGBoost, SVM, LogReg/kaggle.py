...
predictions = svm.predict(eval_selected)



print(f'K-BEST ACC EVAL: {accuracy(eval_anon_labels, predictions)} \n')
...
X_eval_transformed = pipeline.transform(eval_anon_data)
...
y_pred_ensemble = ensemble.predict(X_eval_transformed)


print(f'ACC FOR ENSEMBLE (XGBoost & RandomForest): {accuracy(eval_anon_labels, y_pred_ensemble)} \n')
...
eval_anon_data_basic_predicted_labels_SVM = SVM_basic.predict(eval_anon_data.to_numpy())

print('------BASIC: ------ \n')

print(f'BASIC TEST ACC SVM: {accuracy(test_labels, test_data_basic_predicted_labels_SVM)} \n')
print(f'BASIC EVAL ACC SVM: {accuracy(eval_anon_labels, eval_anon_data_basic_predicted_labels_SVM)} \n')
...
eval_anon_data_basic_predicted_labels_LR = LR_basic.predict(eval_anon_data.to_numpy())

print(f'BASIC TEST ACC LR: {accuracy(test_labels, test_data_basic_predicted_labels_LR)} \n')
print(f'BASIC EVAL ACC LR: {accuracy(eval_anon_labels, eval_anon_data_basic_predicted_labels_LR)} \n')
...
eval_anon_data_threshold_predicted_labels_SVM = SVM_threshold.predict(eval_data_threshold.to_numpy())

print('------THRESHOLD: ------ \n')

print(f'THRESHOLD TEST ACC SVM: {accuracy(test_labels, test_data_threshold_predicted_labels_SVM)} \n')
print(f'THRESHOLD EVAL ACC SVM: {accuracy(eval_anon_labels, eval_anon_data_threshold_predicted_labels_SVM)} \n')
...
eval_anon_data_threshold_predicted_labels_LR = LR_threshold.predict(eval_data_threshold.to_numpy())

print(f'THRESHOLD TEST ACC LR: {accuracy(test_labels, test_data_threshold_predicted_labels_LR)} \n')
print(f'THRESHOLD EVAL ACC LR: {accuracy(eval_anon_labels, eval_anon_data_threshold_predicted_labels_LR)} \n')
...
eval_anon_data_variance_threshold_predicted_labels_SVM = SVM_variance_threshold.predict(eval_anon_variance_threshold.to_numpy())

print('------VARIANCE: ------ \n')

print(f'VARIANCE THRESHOLD TEST ACC SVM: {accuracy(test_labels, test_data_variance_threshold_predicted_labels_SVM)} \n')
print(f'VARIANCE THRESHOLD EVAL ACC SVM: {accuracy(eval_anon_labels, eval_anon_data_variance_threshold_predicted_labels_SVM)} \n')
...
eval_anon_data_variance_threshold_predicted_labels_LR = LR_variance_threshold.predict(eval_anon_variance_threshold.to_numpy())

print(f'VARIANCE THRESHOLD TEST ACC LR: {accuracy(test_labels, test_data_variance_threshold_predicted_labels_LR)} \n')
print(f'VARIANCE THRESHOLD EVAL ACC LR: {accuracy(eval_anon_labels, eval_anon_data_variance_threshold_predicted_labels_LR)} \n')
...
eval_anon_data_standard_scaler_predicted_labels_SVM = SVM_standard_scaler.predict(eval_data_standard_scaler.to_numpy())

print('------STANDARD SCALER: ------ \n')

print(f'STANDARD SCALER TEST ACC SVM: {accuracy(test_labels, test_data_standard_scaler_predicted_labels_SVM)} \n')
print(f'STANDARD SCALER EVAL ACC SVM: {accuracy(eval_anon_labels, eval_anon_data_standard_scaler_predicted_labels_SVM)} \n')
...
eval_anon_data_standard_scaler_predicted_labels_LR = LR_standard_scaler.predict(eval_data_standard_scaler.to_numpy())

print(f'STANDARD SCALER TEST ACC LR: {accuracy(test_labels, test_data_standard_scaler_predicted_labels_LR)} \n')
print(f'STANDARD SCALER EVAL ACC LR: {accuracy(eval_anon_labels, eval_anon_data_standard_scaler_predicted_labels_LR)} \n')
...
eval_anon_data_variance_threshold_predicted_labels_NN = neural_network.predict(eval_data_threshold)

print(f'VARIANCE THRESHOLD TEST ACC NN: {accuracy(test_labels, test_data_variance_threshold_predicted_labels_NN)} \n')
print(f'VARIANCE THRESHOLD EVAL ACC NN: {accuracy(eval_anon_labels, eval_anon_data_variance_threshold_predicted_labels_NN)} \n')
...
print(f'ACCURACY FOR SVM: {accuracy(eval_anon_labels, y_pred)} \n')
...
# Predict on eval set
y_pred_log = logreg.predict(X_eval_scaled)


print(f'ACCURACY FOR LOGREG: {accuracy(eval_anon_labels, y_pred_log)} \n')
...
# Apply SMOTE directly to PCA'd data and original labels
X_resampled, y_resampled = smote.fit_resample(X_pca, train_labels)
...
x_pred_rfc = rfc.predict(standard_scaler.transform(vt.transform(eval_anon_data)))

print(f'Accuracy: {accuracy(eval_anon_labels, x_pred_rfc)}')
...
y_pred = grid.predict(eval_anon_data)

print("\nBest Parameters:")
print(grid.best_params_)

print("\nClassification Report:")
print(classification_report(eval_anon_labels, y_pred))

print(f"F1 Score: {f1_score(eval_anon_labels, y_pred):.4f}")
...
X_eval_scaled = scaler.transform(eval_anon_data)
X_eval_vt = vt.transform(X_eval_scaled)
y_pred = xgb.predict(X_eval_vt)
eval_f1 = f1_score(eval_anon_labels, y_pred)
report = classification_report(eval_anon_labels, y_pred)
...
# Hyperparam testing for SVM & LogReg
...
# Neural Network Hyper-Parameter testing
param_grid = {
...
scaler = RobustScaler()
selector = SelectKBest(f_classif, k='all')  # Start with all features
...
param_grid = {
    'feature_selection__k': [50, 100, 'all'],  # Try different feature counts
...
# Evaluate on test set (eval_anon_data)
if eval_anon_data is not None and eval_anon_labels is not None:
    test_preds = grid_search.predict(eval_anon_data)
    print("\nTest Set Performance:")
    print(classification_report(eval_anon_labels, test_preds))
    print("Confusion Matrix:")
    print(confusion_matrix(eval_anon_labels, test_preds))
...
#     print(np.argsort(importances)[-10:][::-1])
...
eval_anon_prediction = pd.Series(test_preds, name = "label")
...
# print(f'ACC: {accuracy(eval_anon_labels, test_preds)}')